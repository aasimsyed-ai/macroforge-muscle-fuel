      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="flex max-h-[88vh] w-[calc(100vw-1rem)] max-w-md flex-col gap-3 overflow-hidden p-0 sm:max-w-lg">
          <DialogHeader>
            <div className="flex items-center gap-3 border-b border-border bg-secondary/40 px-4 py-3 pr-12">
              ...
              <div className="min-w-0 flex-1 text-left">
                ...
                <p className="mt-0.5 text-[10px] text-muted-foreground">Fitness · food · your progress</p>
              </div>
              <div
                role="radiogroup"
              ...
              messages.map((message) => (
                <Message key={message.id} from={message.role === "user" ? "user" : "assistant"}>
                  <div className="flex items-start gap-2">
                    {message.role === "halku" ? <HalkuHeadshot gender={gender} className="mt-0.5 size-7" /> : null}
                    <MessageContent
                      className={message.isError ? "rounded-md border border-destructive/30 bg-destructive/10 p-2.5" : undefined}
                    >
                      {message.role === "halku" && message.grounded ? (
                        <Badge variant="secondary" className="w-fit text-[10px]">Your data</Badge>
                      ) : null}
          ...
          <p className="mx-4 mb-3 flex items-center gap-1 text-[10px] text-muted-foreground">
            <ShieldCheck className="size-3 text-primary" aria-hidden="true" /> Halku never invents your workout or food data.
          </p>
        </DialogContent>
      </Dialog>
    </>
